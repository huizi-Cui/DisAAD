...
def load_processed_data(file_path):
    ...
    with open(file_path, 'r') as file:
        for idx, line in enumerate(file):
            ...
            if 'bleurt' not in data:
                print(f"[Warning] Line {idx+1} missing 'bleurt' key, skipped.")
                continue
            processed_data.append(data)
    return processed_data

def rank_fun(entry, indicator, k=12, p=5):
    ...
    try:
        if indicator == "topk_prob":
            ...
            if eu and au:
                ...
                return 1/top_k_mean(combined, k)
            return np.nan
...
def calculate_auroc(processed_data, indicator, k):
    ...
    labels = np.array([entry.get('bleurt', np.nan) > 0.50 for entry in processed_data])
...
def calculate_ece_from_confidences(confidences, correctness_list, n_bins=15):
    """Calculate ECE from pre-computed confidences."""
...
def calculate_ece(processed_data, n_bins=15):
    """Calculate Expected Calibration Error (ECE) for sequences"""
    confidences = []
    correctness_list = []
    ...
    for entry in processed_data:
        ...
        if probs:
            ...
            correctness_list.append(entry['bleurt'] > 0.50)
...
def analyze_multiple_models(model_paths, k):
    ...
    for model_name, file_path in model_paths.items():
        ...
        baseline_acc = np.mean([entry['bleurt'] > 0.5 for entry in processed_data])
...
def get_model_paths_config(config_name, num_samples=800):
    ...
    if config_name == "default":
        ...
        return {
            "base_model": f"./results/{dataset_name}/base_model_one_pass_gene_merge_{num_samples}.jsonl",
            "sft_model": f"./results/{dataset_name}/sft_model_one_pass_gene_merge_{num_samples}.jsonl",
        }
    else:
...
if __name__ == "__main__":
    ...
    model_paths_dict = get_model_paths_config(args.config_name, args.num_samples)

    analyze_multiple_models(model_paths_dict, args.k)
